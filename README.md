...
The data sheet of the BME280 strongly recommends to use the original driver from Bosch. For exampe, the compensation parameters should be read in burst mode. Therfore I gave the origimal driver a try. Refer to: [BME280_SensorAPI](https://github.com/boschsensortec/BME280_SensorAPI). The original source code is somewhat old fashioned C language, but ESPHome/platform.io compiles it with no issues.

This BME280I2CWrapper class is just a C++ wrapper component for the original Bosch Sensortec Api code, which is included as library. It uses the I2C interface.
