...
void BME280I2CWrapper::setup() {
  ...
  if (res != BME280_OK) {
    ...
    ESP_LOGD(TAG, "chip_id=%0x02x", dev.chip_id);
  }
  ...
  uint8_t settings_sel = BME280_SEL_OSR_PRESS | BME280_SEL_OSR_TEMP | BME280_SEL_FILTER;

  res = bme280_set_sensor_settings(settings_sel, &settings, &dev);
  ...
  res = bme280_cal_meas_delay(&measure_delay, &settings);
  ...
  if (res != BME280_OK) {
    ...
    ESP_LOGD(TAG, "measure_delay=%d", measure_delay);
  }
...
void BME280I2CWrapper::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  IIR Filter: %s", iir_filter_to_str(this->settings.filter));
  ...
  if (this->temperature_sensor_ != nullptr) {
    ...
    ESP_LOGCONFIG(TAG, "    Oversampling: %s", oversampling_to_str(this->settings.osr_t));
  }
  if (this->humidity_sensor_ != nullptr) {
    LOG_SENSOR("  ", "Humidity", this->humidity_sensor_);
    ESP_LOGCONFIG(TAG, "    Oversampling: %s", oversampling_to_str(this->settings.osr_h));
  }
  if (this->pressure_sensor_ != nullptr) {
    LOG_SENSOR("  ", "Pressure", this->pressure_sensor_);
    ESP_LOGCONFIG(TAG, "    Oversampling: %s", oversampling_to_str(this->settings.osr_p));
  }
}

float BME280I2CWrapper::get_setup_priority() const { return setup_priority::DATA; }

//inline uint8_t oversampling_to_time(BMX280Oversampling over_sampling) { return (1 << uint8_t(over_sampling)) >> 1; }
...
void BME280I2CWrapper::update() {
  ...
  if (forceMeasure()) {
    ...
    getPressureAtSeaLevel();
    getDewPoint(getTemperature(), getHumidity());
  } else {
    ESP_LOGE(TAG, "BME readout failded");
  }
...
double BME280I2CWrapper::getPressureAtSeaLevel() {
  double alti = 0.0065 * altitude_;
  return getPressure() / powf(1 - (altitude_/44330), 5.255); // refer to bmp180 documentation
}

double BME280I2CWrapper::getDewPoint(double temperature, double humidity) {
...
bool BME280I2CWrapper::startMeasure() {
  ...
  do {
    ...
    rslt = bme280_get_regs(BME280_REG_STATUS, &stat, 1, &dev);
